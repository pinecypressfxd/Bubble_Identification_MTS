...
model_dir = '/home/dell/code/project1_MultiTS/model_result_train_test_validation_reevaluate/resnet2D_result_NP_ratio_40/val_86_82_test_83_80_choose/'
    ...
    for i_var in range(0,len(variance_type)-2):
        for i_normalized in range(1,len(normalized_type)-1):
            ...
            for i_step in range(0,len(step)):
                ...
                for i_year in range(0,len(year_list)):#test:2020
                    ...
                    for i_thm in range(0,len(thm)):#test:tha
                        ...
                        for i_month in range(0,len(source_file_time)):#读取每月数据
                            ...
                            # store.close()
                            predict_data_inital = predict_data_inital.reshape(np.shape(predict_data_inital)[0],240,int(np.shape(predict_data_inital)[1]/240))
                            predict_data_inital = predict_data_inital.reshape((predict_data_inital.shape[0], predict_data_inital.shape[1],predict_data_inital.shape[2], 1))
                ...
                bubble_list_file_name = 'resnet-20220110_predicted_bubble_list-NP-Ratio-'+str(NP_ratio[i_NP_ratio])+'-'+variance_type[i_var]+'-'+normalized_type[i_normalized]+'-'+stepname[i_step]+'.csv'
                bubble_time_list.to_csv(out_result_path+bubble_list_file_name)
                            
                all_bubble_list_file_name = 'all_resnet-20220110_predicted_bubble_list-NP-Ratio-'+str(NP_ratio[i_NP_ratio])+'-'+variance_type[i_var]+'-'+normalized_type[i_normalized]+'-'+stepname[i_step]+'.csv'
